...
В ИИ-чате «Сфотографировать» открывает встроенный экран камеры с предпросмотром, переключением передней/задней камеры и кнопкой снимка. Используется `getUserMedia` без микрофона; необходим HTTPS и разрешение камеры для Telegram и мини-приложения. Снимок прикрепляется к сообщению и отправляется только кнопкой отправки. При закрытии камеры, уходе с вкладки чата или сворачивании мини-приложения видеопоток выключается. Если Telegram/WebView не поддерживает доступ или пользователь его запретил, показывается причина и кнопка повтора; галерея открывается только отдельным действием. [Описание доступа к камере](https://developer.mozilla.org/en-US/docs/Web/API/MediaDevices/getUserMedia).
