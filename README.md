...
В ИИ-чате кнопка фото открывает выбор «Сфотографировать» / «Выбрать из галереи». Камера вызывается системным выбором файла с `capture="environment"`; её доступность зависит от Telegram/WebView на устройстве. На компьютере без поддержки захвата браузер может открыть обычный выбор файла. [Описание HTML capture](https://developer.mozilla.org/en-US/docs/Web/HTML/Reference/Attributes/capture).
