...
- Ответ портала «Для заданных параметров данные не могут быть предоставлены» отображается отдельным сообщением: он не подтверждает отсутствие занятий. Такое состояние и пустая неделя кэшируются на 5 минут. Mini App прекращает ожидание не позднее чем примерно через 20 секунд и предлагает повторить запрос; расписание другой группы не подставляется. При обновлении парсера пересобирайте вместе `bot miniapp dashboard worker`, чтобы все сервисы использовали одну версию кэша.
